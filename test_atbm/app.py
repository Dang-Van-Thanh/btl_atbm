...
if os.getenv("USE_WINDOWS_AUTH") == "True":
    app.config['SQLALCHEMY_DATABASE_URI'] = (
        f"mssql+pyodbc://@{os.getenv('DB_SERVER')}/{os.getenv('DB_NAME')}?driver={os.getenv('DB_DRIVER')}&trusted_connection=yes"
    )
else:
    app.config['SQLALCHEMY_DATABASE_URI'] = (
        f"mssql+pyodbc://{os.getenv('DB_USER')}:{os.getenv('DB_PASSWORD')}@{os.getenv('DB_SERVER')}/"
        f"{os.getenv('DB_NAME')}?driver={os.getenv('DB_DRIVER')}"
    )
